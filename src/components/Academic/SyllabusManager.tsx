  ...
  return (
    <div className="space-y-8">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          <h3 className="text-3xl font-black text-slate-800 tracking-tight">Syllabus Management</h3>
          <p className="text-slate-500 font-bold mt-1">Create and distribute academic syllabus across branches</p>
        </div>
        {(role === 'SUPER_ADMIN' || role === 'SCHOOL_ADMIN') && (
          <Button icon={Plus} size="lg" className="shadow-xl shadow-brand-100">
            Create New Syllabus
          </Button>
        )}
      </div>
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-10">
          ...
          <div className="flex items-center gap-3">
            <select 
              ...
              className="bg-slate-50 border-none rounded-xl px-4 py-3.5 text-sm font-black text-slate-600 outline-none shadow-sm cursor-pointer"
            >
              <option value="ALL">All Classes</option>
              {CLASSES.map(c => <option key={c.id} value={c.id}>{c.name}</option>)}
            </select>
            <Button variant="outline" className="p-3.5 bg-slate-50 border-transparent">
              <Filter size={20} />
            </Button>
          </div>
        </div>
          ...
          <AnimatePresence mode="popLayout">
            {filteredSyllabus.map((syl, idx) => (
              <motion.div
                ...
                key={syl.id}
                className="group p-8 rounded-[2.5rem] bg-slate-50 border-2 border-transparent hover:border-brand-100 hover:bg-white hover:shadow-premium transition-all cursor-pointer"
              >
            ...
            >
              <div className="p-10 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
                <div className="flex items-center gap-5">
                  <div className="w-14 h-14 rounded-2xl bg-brand-500 text-white flex items-center justify-center shadow-xl shadow-brand-100">
                    <Share2 size={28} />
                  </div>
                  <div>
                    <h3 className="text-2xl font-black text-slate-900 tracking-tight">Assign Syllabus</h3>
                    <p className="text-sm text-slate-500 font-bold mt-1">{activeSyllabus.title}</p>
                  </div>
                </div>
                <button 
                  onClick={() => setIsAssignModalOpen(false)}
              ...
              <div className="p-10 border-t border-slate-100 bg-slate-50/50 flex justify-end gap-4">
                <Button variant="outline" onClick={() => setIsAssignModalOpen(false)} className="bg-white">
                  Cancel
                </Button>
                <Button className="px-10 shadow-xl shadow-brand-100">
                  Save Assignments
                </Button>
              </div>
            </motion.div>
